...
fn create_printnode_tree(root: ASTNode) -> PrintNode {
    ...
    let print_root = match root {
        ...
        IntegerLiteral { token, val } => PrintNode { s: token.lexeme.clone(), c: Vec::new() },
        FloatLiteral   { token, val } => PrintNode { s: token.lexeme.clone(), c: Vec::new() },
        StringLiteral  { token, val } => PrintNode { s: token.lexeme.clone(), c: Vec::new() },
        BoolLiteral    { token, val } => PrintNode { s: token.lexeme.clone(), c: Vec::new() },
        Identifier     { token, name} => PrintNode { s: token.lexeme.clone(), c: Vec::new() },
        ...
        Grouping { expr, left_delim, right_delim } => create_printnode_tree(expr.as_ref().clone()),
